...
#if F = 1 && M == 1 { KLESS_accl_en=1; KLESS_replicate_accl_en = 0;  KLESS_multithreaded_accl_en=1}
#if F = 3 && M == 3 { KLESS_accl_en=1; KLESS_replicate_accl_en = 1;  KLESS_multithreaded_accl_en=0}
#if F = 1 && M == 3 { KLESS_accl_en=1; KLESS_replicate_accl_en = 1;  KLESS_multithreaded_accl_en=1}
...
then
	...
	KLESS_morph_en=0
    KLESS_fetch_stage_en=0       	# Enables the generation of a fetch stage buffer, else the incoming instrution will go directly to the decode stage.
    KLESS_branch_predict_en=0     	# This enables the branch predictor
    KLESS_btb_en=0   				# Enables the BTB instead of the single bit predictor
    KLESS_btb_len=6 				# Indicates the number of entries in the btb which is 2^btb_len
